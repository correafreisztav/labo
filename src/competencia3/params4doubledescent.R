...
#----------------- aca empieza la adaptacion de la bayesiana para NO HACER BAYESIANA


#si usted es ambicioso, y tiene paciencia, podria subir este valor a 100
kBO_iteraciones  <- 2  #iteraciones de la Optimizacion Bayesiana
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  param_completo$early_stopping_rounds  <- as.integer(2000 + 4/param_completo$learning_rate ) #aca modificar para el DoubleDescent
...
EstimarGanancia_lightgbmCV  <- function( x )
{
  ...
  param_completo$early_stopping_rounds  <- as.integer(2000 + 4/param_completo$learning_rate )
...
if( file.exists( "DD_log.txt" ) )
{
  tabla_log  <- fread( "DD_log.txt" )
